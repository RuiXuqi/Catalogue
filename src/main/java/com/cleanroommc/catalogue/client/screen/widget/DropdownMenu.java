...
public class DropdownMenu extends Gui {
    ...
    public void toggle(GuiButton widget) {
        this.toggle(new Anchor(widget.xPosition, widget.yPosition, widget.width, widget.height));
    }
    ...
    private static class MenuItem extends Gui {
        private static final ResourceLocation TEXTURE = Catalogue.resource("textures/gui/sprites/dropdown/item.png");
        private static final ResourceLocation HIGHLIGHTED_TEXTURE = Catalogue.resource("textures/gui/sprites/dropdown/item_highlighted.png");
    ...
    private static class CheckboxMenuItem extends MenuItem {
        ...
        protected void drawWidget(Minecraft minecraft, int mouseX, int mouseY, float deltaTick) {
            ...
            GL11.glEnable(GL11.GL_BLEND);
            OpenGlHelper.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA, GL11.GL_ONE, GL11.GL_ZERO);
            RenderUtils.drawModalRectWithCustomSizedTexture(this.x + this.width - 14 - offset, this.y + offset, this.hovered ? 14 : 0, this.holder.getValue() ? 14 : 0, 14, 14, 64, 64);
        }
