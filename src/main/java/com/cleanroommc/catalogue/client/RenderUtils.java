...
public final class RenderUtils {
    ...
    public static void drawScrollingString(
        ...
        if (textWidth > boxWidth) {
            ...
            int clampedX = MathHelper.clamp_int(centerX, minX + textWidth / 2, maxX - textWidth / 2);
    ...
    private static void innerBlit(int left, int right, int top, int bottom, int z, float minU, float maxU, float minV, float maxV) {
        Tessellator tess = Tessellator.instance;
        tess.startDrawingQuads();
        tess.addVertexWithUV(left, top, z, minU, minV);
        tess.addVertexWithUV(left, bottom, z, minU, maxV);
        tess.addVertexWithUV(right, bottom, z, maxU, maxV);
        tess.addVertexWithUV(right, top, z, maxU, minV);
        tess.draw();
    }
    ...
    public static void drawModalRectWithCustomSizedTexture(int x, int y, float u, float v, int width, int height, float texWidth, float texHeight) {
        Gui.func_146110_a(x, y, u, v, width, height, texWidth, texHeight);
    }

    public static void drawScaledCustomSizeModalRect(int x, int y, float u, float v, int uWidth, int vHeight, int width, int height, float texWidth, float texHeight) {
        Gui.func_152125_a(x, y, u, v, uWidth, vHeight, width, height, texWidth, texHeight);
    }
}
